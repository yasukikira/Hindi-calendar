    ...
    <div className="flex flex-col items-center justify-center p-4 bg-gray-900 rounded-xl text-white shadow-lg border border-gray-700">
      <div className="relative w-16 h-16 rounded-full bg-gray-800 overflow-hidden shadow-inner border border-gray-600">
        <div 
          ...
          style={{ opacity: Math.max(0.1, phase), transform: `scale(${0.2 + (phase * 0.8)})` }}
        />
    ...
    if (navigator.geolocation) {
      ...
      navigator.geolocation.getCurrentPosition(
        (pos) => {
          ...
          calcTimes(pos.coords.latitude, pos.coords.longitude);
          setLocLoading(false);
        },
        () => {
          calcTimes(defaultLoc.lat, defaultLoc.lng);
          setLocLoading(false);
        }
      );
    } else {
...
Check: https://yasukikira.github.io/Hindi-calendar/
  ...
  // DETERMINE HEADER CLASS FOR ANIMATION
  const isFestive = theme && ['national', 'diwali', 'holi', 'rakhi', 'ganesh', 'onam', 'navratri', 'christmas', 'eid', 'bakrid', 'muharram', 'milad', 'newyear', 'sankranti'].includes(theme.type);
  ...
  return (
    <div className={`fixed inset-y-0 right-0 w-full md:w-[400px] shadow-2xl z-50 animate-slide-in flex flex-col font-sans ${darkMode ? 'bg-gray-900' : 'bg-white'}`}>
      ...
      {/* Header with Dynamic Theme Class */}
      <div className={`relative p-6 text-white overflow-hidden shrink-0 transition-colors duration-500 ${headerClass}`}>
        {isFestive && <Confetti />}
        ...
        <div className="relative z-10 text-white">
          ...
          <div className="flex items-center gap-2 mt-4 opacity-90 text-sm uppercase tracking-widest font-bold">
            ...
            <span>•</span>
            <span>{theme ? theme.name : relativeLabel}</span>
          </div>
        </div>
      </div>
          ...
          <div className="space-y-6 animate-fade-in">
             ...
             <div className="grid grid-cols-2 gap-4">
                <div className={`p-4 rounded-xl border shadow-sm flex flex-col items-center justify-center text-center ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
                   ...
                   <span className="text-xs opacity-70 uppercase">{t.ui.sunrise}</span>
                   <span className="font-bold">{sunTimes ? formatTime(sunTimes.sunrise) : '--:--'}</span>
                </div>
                <div className={`p-4 rounded-xl border shadow-sm flex flex-col items-center justify-center text-center ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
                   <Moon size={24} className="text-indigo-400 mb-2" />
                   <span className="text-xs opacity-70 uppercase">{t.ui.sunset}</span>
                   <span className="font-bold">{sunTimes ? formatTime(sunTimes.sunset) : '--:--'}</span>
                </div>
             </div>
             ...
             <div className={`rounded-2xl p-6 shadow-sm border ${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-100'}`}>
              <div className="flex justify-between items-center mb-4">
                <h3 className={`font-bold flex items-center gap-2 ${lang !== 'en' ? 'font-hindi' : 'font-eng'}`}>
                  <CalendarIcon size={18} className="text-blue-500" /> {t.ui.events}
                </h3>
                <div className="flex gap-2">
                  <button onClick={handleShare} className="text-green-500 hover:bg-green-500/10 p-2 rounded-full transition-colors" title={t.ui.share}>
                    <MessageCircle size={18} />
                  </button>
                  <button onClick={addToGoogleCalendar} className="text-blue-500 hover:bg-blue-500/10 p-2 rounded-full transition-colors" title={t.ui.addToCal}>
              ...
              <div className="space-y-3 mb-4">
                {events && events.length > 0 ? (events.map((evt, i) => (
                  <div key={i} className={`flex items-start gap-3 p-3 rounded-lg border ${darkMode ? 'bg-blue-900/20 border-blue-800' : 'bg-blue-50 border-blue-100'}`}>
                 ...
                 <div className={`space-y-6 ${lang !== 'en' ? 'font-hindi' : 'font-eng'}`}>
                  {[
                    ...
                    <div key={i} className={`flex justify-between items-center border-b pb-3 ${darkMode ? 'border-gray-700' : 'border-gray-50'}`}>
                      <span className="opacity-60 text-sm">{item.l}</span>
                      <span className="font-semibold text-lg">{item.v}</span>
                    </div>
                  ))}
                 </div>
              </>
              ...
              <div className="space-y-2">
                 {choghadiya.map((c, i) => (
                  <div key={i} className={`flex items-center justify-between text-sm py-3 border-b ${darkMode ? 'border-gray-700' : 'border-gray-50'}`}>
                    <span className="opacity-60 font-mono text-xs">{c.time}</span>
                    <div className="flex items-center gap-2">
                        <span className={`font-medium ${lang !== 'en' ? 'font-hindi' : 'font-eng'}`}>{c.label}</span>
                        <div className={`w-2.5 h-2.5 rounded-full ${c.quality === 'good' ? 'bg-green-500' : c.quality === 'bad' ? 'bg-red-500' : 'bg-gray-400'}`}></div>
                    </div>
                  </div>
                ))}
              </div>
