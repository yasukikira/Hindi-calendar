...
import { X, Sun, Moon, Calendar as CalendarIcon, Clock, MapPin, Share2, AlertTriangle, Send } from 'lucide-react';
...
const DetailPanel = ({ date, onClose, events, onAddEvent, lang }) => {
  ...
  const handleShare = async () => {
    const title = lang === 'hi' ? `📅 आज का पंचांग` : `📅 Daily Panchang`;
    const text = `
${title} • ${date.toDateString()}
------------------------
✨ ${theme ? theme.name : 'No Festival'}
🌕 ${tithiName}, ${pakshaName}
🌟 ${nakshatraName}
🌞 Sunrise: ${sunTimes ? formatTime(sunTimes.sunrise) : '--:--'}
🚫 Rahu Kaal: ${auspicious ? auspicious.rahu : '--:--'}
  ...
  return (
    <div className="fixed inset-y-0 right-0 w-full md:w-[400px] bg-white shadow-2xl z-50 animate-slide-in flex flex-col font-sans">
      ...
      <div className="flex border-b border-gray-100 bg-white">
        {['overview', 'panchang', 'muhurat'].map((tab) => (
          <button
            ...
            className={`flex-1 py-3 text-sm font-medium transition-colors relative ${activeTab === tab ? 'text-orange-600 bg-orange-50' : 'text-gray-500 hover:bg-gray-50'} ${lang === 'hi' ? 'font-hindi' : 'font-eng'}`}
          >
            {t.ui.tabs[tab]}
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50/50 p-6">
               ...
               <div className="grid grid-cols-2 gap-3">
                 <div className="bg-red-50 p-3 rounded-xl border border-red-100 flex flex-col items-center justify-center text-center">
                   <div className="flex items-center gap-1 text-red-600 mb-1">
                     ...
                     <span className="text-[10px] font-bold uppercase tracking-wider">Rahu Kaal</span>
                   </div>
                 ...
                 <div className="bg-orange-50 p-3 rounded-xl border border-orange-100 flex flex-col items-center justify-center text-center">
                   <span className="text-[10px] font-bold uppercase tracking-wider text-orange-600 mb-1">Yamaganda</span>
              ...
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="flex gap-2">
                  <button 
                    ...
                    title={lang === 'hi' ? "शेयर करें" : "Share Panchang"}
                  >
                    <Send size={16} />
                  </button>
                  <button 
                    ...
                    title={lang === 'hi' ? "गूगल कैलेंडर" : "Google Calendar"}
                  >
                    <Share2 size={16} />
                  </button>
                </div>
        ...
        {/* PANCHANG TAB & MUHURAT TAB CONTENT (Same as before) */}
        {activeTab === 'panchang' && (
